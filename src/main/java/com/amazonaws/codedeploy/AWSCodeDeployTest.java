...
/**
 * The AWS CodeDeploy Publisher is a post-build plugin that adds the ability to
 * start a new CodeDeploy deployment with the project's workspace as the
 * application revision.
 *
 * To configure, users must create an IAM role that allows "S3" and "CodeDeploy"
 * actions and must be assumable by the globally configured keys. This allows
 * the plugin to get temporary credentials instead of requiring permanent
...
//	private Map<String, String> envVars;
	...
	public AWSCodeDeployTest(String s3bucket, String s3prefix, String githubRepository, String githubCommitId,
			...
			String credentials, String versionFileName, String deploymentMethod, String awsAccessKey,
			String awsSecretKey, String iamRoleArn, String externalId, String includes, String proxyHost, int proxyPort,
			String excludes, String subdirectory) {
	...
	public void perform() throws IOException, InterruptedException {
		...
		if ("awsAccessKey".equals(credentials)) {
			if (StringUtils.isEmpty(this.awsAccessKey) && StringUtils.isEmpty(this.awsSecretKey)) {
				...
				aws = AWSClients.fromBasicCredentials(this.region, this.awsAccessKey, this.awsSecretKey, this.proxyHost,
						this.proxyPort);
			}
		} else {
			aws = AWSClients.fromIAMRole(this.region, this.iamRoleArn, this.externalId, this.proxyHost, this.proxyPort);
		}
...
//	}
	...
	private void verifyCodeDeployApplication(AWSClients aws) throws IllegalArgumentException {
		...
		ListDeploymentGroupsResult deploymentGroups = aws.codedeploy
				.listDeploymentGroups(new ListDeploymentGroupsRequest().withApplicationName(applicationName));
	...
	private RevisionLocation zipAndUpload(AWSClients aws, String projectName, String sourceDirectory)
...
//	}
	...
	private void registerRevision(AWSClients aws, RevisionLocation revisionLocation) {
		...
		aws.codedeploy.registerApplicationRevision(
				new RegisterApplicationRevisionRequest().withApplicationName(applicationName)
						.withRevision(revisionLocation).withDescription("Application revision registered via Jenkins"));
	}
	...
	private String createDeployment(AWSClients aws, RevisionLocation revisionLocation) throws Exception {
		...
		CreateDeploymentResult createDeploymentResult = aws.codedeploy
				.createDeployment(new CreateDeploymentRequest().withDeploymentConfigName(getDeploymentConfigFromEnv())
						.withDeploymentGroupName(getDeploymentGroupNameFromEnv())
						.withApplicationName(getApplicationNameFromEnv()).withRevision(revisionLocation)
	...
	private boolean waitForDeployment(AWSClients aws, String deploymentId) throws InterruptedException {
		...
		logger.println("Deployment status: " + deployStatus.getStatus() + "; instances: "
	...
	/**
	 *
	 * Descriptor for {@link AWSCodeDeployPublisher}. Used as a singleton. The class
	 * is marked as public so that it can be accessed from views.
	 *
	 * See
	 * <tt>src/main/resources/com/amazonaws/codedeploy/AWSCodeDeployPublisher/*.jelly</tt>
...
//		}
//
//		public FormValidation doCheckName( String value) throws IOException, ServletException {
//			if (value.length() == 0)
